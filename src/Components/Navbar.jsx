...
const Navbar = () => {
  ...
  const handleClick = (item) => {
    if (item === "LinkedIn") {
      window.location.href = "https://www.linkedin.com/in/arpit-kr-b44b8521a/";
    } else {
  ...
  return (
    <div
      className={`fixed z-[999] ${isScrolled ? 'bg-transparent' : 'bg-zinc-900'} flex w-full px-20 py-8 font-neue justify-between items-center transition-all duration-300`}
    >
      {/* Logo click event */}
      <div id="Logo" onClick={() => window.location.href = 'https://github.com/ArpitKrRespect/Ochi'}>
        <a>
          <svg
            width="72"
            height="30"
      ...
      <div id="links" className="flex gap-10">
        {["Servics", "Our Contact", "About Us", "Insights", "LinkedIn"].map(
          (item, index) => (
            <a
              key={index}
              onClick={() => handleClick(item)}
              className={`text-md font-semibold capitalize cursor-pointer hover:text-blue-500 transition-all duration-200 ${index === 4 && "ml-32"}`}
            >
              {item}
            </a>
          )
